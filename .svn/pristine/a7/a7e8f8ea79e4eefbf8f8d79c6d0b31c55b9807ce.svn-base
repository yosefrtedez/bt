...
uses UDM, UFTanggal, UMain, RzStatus, UTransferKas;
      ...
      if Frm_TransferKas.ShowModal = mrok then begin
        ...
        if MessageDlg('Cetak ?',mtConfirmation,[mbYes,mbNo],0)=mryes then begin
          ...
          if IsEmpty then begin
            ...
            ParamByName('c').Value := DM.ConvKeHuruf(IntToStr(Frm_TransferKas.txtjumlah.IntValue));
            ExecSQL;
          end else begin
            Close;
            SQL.Clear;
    ...
    if QData.FieldValues['approve'] = 0 then begin
      if MessageDlg('Transfer Kas/Bank telah disetujui ?',mtConfirmation,[mbYes,mbNo],0)=mryes then begin
        with TZQuery.Create(Self)do begin
          ...
          DM.InsertBukuBesarAkun(FieldValues['nokasketransit'],Date,'TR',QData.FieldValues['noreferensi'],'Transfer Kas/Bank',FieldValues['nilai'],0);
          DM.InsertBukuBesarAkun(FieldValues['nokastransit'],Date,'TR',QData.FieldValues['noreferensi'],'Transfer Kas/Bank',0,FieldValues['nilai']);
