...
begin
  ...
  with dbgpembayaran do begin
    ...
    if total = 0 then
      txtterbilang.Clear
    else txtterbilang.Text := DM.ConvKeHuruf(IntToStr(txtsebesar.IntValue));
  end;
end;
    ...
    if MessageDlg('Apakah data sudah benar ?',mtConfirmation,[mbYes,mbNo],0)=mryes then begin
      with TZQuery.Create(Self) do begin
        ...
        try
          ...
          Close;
          SQL.Clear;
          SQL.Text := 'insert into tbl_pembayaranhutang values (:a,:b,:c,:d,:e,:f,:g,:h,:i,:j,:k,:l)';
...
procedure TFrm_PembayaranHutang.dtptanggalChange(Sender: TObject);
begin
