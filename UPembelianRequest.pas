...
type
  TFrm_PembelianRequest = class(TForm)
    ...
    procedure dbgbarangSelectionChanged(Sender: TObject; ACol,
      ARow: Integer);
    procedure dtpdibuatChange(Sender: TObject);
  private
...
begin
  ...
  if Frm_DataBarang.ShowModal = mrok then begin
    ...
    for j:=0 to i-1 do begin
      if (dbgbarang.Cell[8,j].AsInteger = Frm_DataBarang.QData.FieldValues['nobarang']) and (dbgbarang.Cell[9,j].AsInteger = Frm_DataBarang.QData.FieldValues['nosatuan']) then begin
        Exit;
      end;
    end;
    ...
    if LID.Caption = '0' then begin
      if MessageDlg('Apakah data sudah benar ?',mtConfirmation,[mbYes,mbNo],0)=mryes then begin
        ...
        try
          ...
          SQL.Clear;
          SQL.Text := 'insert into tbl_pembelianrequest values (:a,:b,:c,:f,:g,:h)';
      ...
      if LOrder.Caption = '0' then begin
        if MessageDlg('Apakah data sudah benar ?',mtConfirmation,[mbYes,mbNo],0)=mryes then begin
          ...
          try
            ...
            SQL.Text := 'update tbl_pembelianrequest set tglpembelianrequest=:b,selesai=:d,catatan=:h where nopembelianrequest=:a';
...
procedure TFrm_PembelianRequest.dtpdibuatChange(Sender: TObject);
begin
